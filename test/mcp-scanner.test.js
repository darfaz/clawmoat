...
describe('MCP Scanner', () => {
  test('detects bash as MCP command', () => {
    ...
    assert.ok(findings.some(f => f.id === 'mcp-arbitrary-cmd'));
  ...
  test('detects root filesystem access', () => {
    ...
    assert.ok(findings.some(f => f.id === 'mcp-root-fs'));
  });
  ...
  test('detects credentials in env vars', () => {
    ...
    assert.ok(findings.some(f => f.id === 'mcp-env-creds'));
  });
  ...
  test('detects external URLs', () => {
    ...
    assert.ok(findings.some(f => f.id === 'mcp-external-url'));
  });
  ...
  test('discoverMCPConfigs returns expected paths', () => {
    ...
    assert.ok(configs.some(c => c.name.includes('Claude')));
    assert.ok(configs.some(c => c.name.includes('Cursor')));
  });
